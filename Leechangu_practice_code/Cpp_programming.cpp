...
private:
	...
	int* mem; 
protected:
	_BaseArray_(int capacity = 100) {
		this->capacity = capacity; 
		mem = new int[capacity]; 
	}
	~_BaseArray_() { delete[] mem; }
	void put(int index, int val) { mem[index] = val; }
	int get(int index) { return mem[index]; }
	int getCapacity() { return capacity; }
};
class MyQueue :public _BaseArray_{
	int size;
public:
	MyQueue() { size = 0;}
	MyQueue(int cap) : _BaseArray_(cap) {
		this->size = 0;
	}
	int length()
	{
...
class Oval: public Shape{
   int A,B;
public:
   Oval(string name, int x, int y): Shape(name){
      this->A=x;
      this->B=y;
   }
   float getArea(){ return 3.14*A*B;}
};
class _Rect: public Shape{
   int A,B;
public:
   _Rect(string name, int x, int y): Shape(name){
      this->A=x;
      this->B=y;
   }
...
class Triangular: public Shape{
   int A,B;
public:
   Triangular(string name, int x, int y): Shape(name){
      this->A=x;
      this->B=y;
   }
...
int main() {
    ...
    p[1] = new _Rect("찰떡", 30, 40);         // upcasting 
...
class GoodCalc: public Calculator_{
public:
    GoodCalc(){
      this->Set();
    };
...
class Subtractor_: public Calculator_{
public:
    Subtractor_(){};
    int calc(int a, int b){
      return a-b;
